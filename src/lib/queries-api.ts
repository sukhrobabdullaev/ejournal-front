...
import { apiClient, User, Submission, ReviewAssignment, TopicArea, EditorialBoardMember, TokenManager } from "./api";
...
export async function login(
  ...
  const result = await apiClient.post<{ access: string; refresh: string }>("/auth/login", { email, password });
...
export async function submitSubmission(id: string): Promise<{ data: Submission | null; error: any }> {
  return await apiClient.post<Submission>(`/submissions/${id}/submit/`, {});
}

export async function resubmitSubmission(id: string): Promise<{ data: Submission | null; error: any }> {
...
export async function acceptReviewInvitation(id: string): Promise<{ data: ReviewAssignment | null; error: any }> {
  return await apiClient.post<ReviewAssignment>(`/reviewer/assignments/${id}/accept/`, {});
}

export async function declineReviewInvitation(id: string): Promise<{ data: ReviewAssignment | null; error: any }> {
...
export async function submitReview(
  ...
  return await apiClient.post<ReviewAssignment>(`/reviewer/assignments/${id}/submit-review/`, reviewData);
}

export async function getAssignmentByToken(token: string): Promise<ReviewAssignment | null> {
  const { data, error } = await apiClient.get<ReviewAssignment>(`/reviewer/accept-by-token/?token=${token}`);
  if (error) {
...
export async function deskReject(id: string, reason: string): Promise<{ data: Submission | null; error: any }> {
...
export async function publishSubmission(id: string): Promise<{ data: Submission | null; error: any }> {
...
export async function rejectReviewer(userId: number, reason: string): Promise<{ data: any; error: any }> {
  return await apiClient.post(`/admin/users/${userId}/reject-reviewer/`, {
    reason,
  });
}

export async function rejectEditor(userId: number, reason: string): Promise<{ data: any; error: any }> {
...
export async function uploadFile(file: File): Promise<{ data: { url: string } | null; error: any }> {
  const formData = new FormData();
  formData.append("file", file);
