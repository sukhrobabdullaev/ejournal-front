...
export function CertificatePublic() {
  ...
  const { data: certificate, isLoading } = useQuery({
    ...
    queryFn: () => (code ? getPublicCertificateByCode(code) : Promise.resolve(null)),
    enabled: !!code,
    retry: false,
  });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#F5F9FF] to-[#EEF4FD] px-4 py-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="rounded-md border border-[#8BC0D6] bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,0.08)] md:p-10">
          <div className="mb-8 text-center">
            <p className="mb-2 text-sm font-semibold tracking-[0.2em] text-[#0E6B94]">DCIJ INFO</p>
            <h1 className="text-3xl font-bold text-[#0E6B94] md:text-4xl">
              CERTIFICATE OF REVIEWER RECOGNITION
            </h1>
          </div>

          <div className="mb-10 text-center">
            <p className="text-lg font-semibold text-slate-900">This is to certify that</p>
            <p className="mt-2 text-4xl font-bold text-[#1F5F95]">{certificate.author_name}</p>
            <p className="mt-5 text-lg font-semibold text-[#2B2B5A]">
              has received a positive review recommendation for:
            </p>
            <p className="mx-auto mt-3 max-w-3xl text-2xl text-slate-900">
              {certificate.submission_title}
            </p>
          ...
          <div className="grid gap-6 border-t border-[#D8E4F6] pt-6 md:grid-cols-2">
            <div className="rounded-md border border-[#D8E4F6] bg-[#F8FBFF] p-4">
              ...
              <p className="mt-2 text-xs text-slate-600">
                QR kodni skaner qilsangiz ushbu sertifikat sahifasi ochiladi.
              </p>
            </div>
